...
public class DatabaseManager {
    ...
    private final Map<String, List<RegionData>> checkpointsCache = new ConcurrentHashMap<>();
    private final Map<String, Integer> checkpointCountCache = new ConcurrentHashMap<>();
    ...
    public DatabaseManager(FormulaRacing plugin, FileManager fileManager) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("[FormulaRacing] FALHA CRÍTICA ao conectar ao banco de dados: " + e.getMessage());
    ...
    public synchronized Connection getOrConnect() throws SQLException {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            try {
                ...
                if (
                        e.getMessage() != null &&
                                (e.getMessage().contains("database is locked") || e.getMessage().contains("SQLITE_BUSY"))
                ) {
                    try {
    ...
    public CompletableFuture<Boolean> savePitStopStartRegion(String trackName, double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
        return CompletableFuture.supplyAsync(() -> {
            // Usamos UPDATE para preencher apenas as colunas de START onde a pista coincide
            String sql = "UPDATE fr_pit_stops SET startMinX = ?, startMinY = ?, startMinZ = ?, " +
                    "startMaxX = ?, startMaxY = ?, startMaxZ = ? WHERE trackNameWS = ?";

            try (Connection conn = this.getOrConnect();
                ...
                if (affectedRows == 0) {
                    this.plugin.getDebugManager().logDatabaseOperation("Aviso: Nenhuma pista encontrada com o nome " + trackName + " para salvar o Start.");
                    return false;
                }

                return true;
            } catch (SQLException e) {
                this.plugin.getDebugManager().logDatabaseOperation("Erro ao salvar região Start no banco: " + e.getMessage());
                e.printStackTrace();
                return false;
            }
        });
     ...
     */
    public boolean updateDrsRegion(String trackName, String type, Location min, Location max) {
        ...
        String sql = "UPDATE fr_tracks SET " + columnMin + " = ?, " + columnMax + " = ? WHERE track_name = ?;";
        ...
        try (Connection conn = getOrConnect();
            ...
            Bukkit.getLogger().severe("[FormulaRacing] Erro ao atualizar DRS (" + type + "): " + e.getMessage());
            return false;
        }
    }
    public boolean saveOrUpdateDrsZone(String trackName, String type, Location min, Location max) {
        ...
        if (!exists) {
            ...
            sql = "INSERT INTO fr_drs (trackNameWS, world, " +
                    prefix + "MinX, " + prefix + "MinY, " + prefix + "MinZ, " +
                    prefix + "MaxX, " + prefix + "MaxY, " + prefix + "MaxZ) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
        } else {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            if (!exists) {
                ...
                ps.setDouble(3, min.getX()); ps.setDouble(4, min.getY()); ps.setDouble(5, min.getZ());
                ps.setDouble(6, max.getX()); ps.setDouble(7, max.getY()); ps.setDouble(8, max.getZ());
            } else {
                ps.setString(1, min.getWorld().getName());
                ps.setDouble(2, min.getX()); ps.setDouble(3, min.getY()); ps.setDouble(4, min.getZ());
                ps.setDouble(5, max.getX()); ps.setDouble(6, max.getY()); ps.setDouble(7, max.getZ());
                ps.setString(8, trackName);
            }
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            Bukkit.getLogger().severe("[FormulaRacing] Erro ao salvar DRS: " + e.getMessage());
            return false;
        }
    }
    /**
    ...
    public boolean clearAllDrsRegions(String trackName) {
        ...
        try (Connection conn = getOrConnect();
            ...
            Bukkit.getLogger().severe("[FormulaRacing] Erro ao deletar regiões de DRS da pista " + trackName + ": " + e.getMessage());
            return false;
        }
    }
     ...
     */
    public CompletableFuture<Boolean> savePitStopStart(String trackName, Location min, Location max) {
        return CompletableFuture.supplyAsync(() -> {
            String sql = "UPDATE fr_pit_stops SET " +
            ...
            try (Connection conn = this.getOrConnect();
                ...
                this.plugin.getDebugManager().logDatabaseOperation("Erro ao salvar região Start: " + e.getMessage());
                e.printStackTrace();
                return false;
            }
        });
    ...
    private void initDatabase(Connection conn) {
        try (Statement stmt = conn.createStatement()) {
            ...
            try {
                stmt.execute("ALTER TABLE fr_players ADD COLUMN lang STRING DEFAULT NULL;");
            } catch (SQLException ignored) {}
            try {
                stmt.execute("ALTER TABLE fr_players ADD COLUMN selectedHeat INTEGER DEFAULT NULL;");
            } catch (SQLException ignored) {}

            // Migração segura para a coluna winner
            try {
                stmt.execute("ALTER TABLE fr_timetrial_duels ADD COLUMN winner VARCHAR(36) NULL;");
            } catch (SQLException ignored) {}
            ...
            stmt.executeUpdate("CREATE TABLE IF NOT EXISTS fr_drs (" +
            ...
            for (String col : newColumns) {
                try {
                    stmt.executeUpdate("ALTER TABLE fr_pit_stops ADD COLUMN " + col + " REAL");
                } catch (SQLException ignored) {
            ...
            String[] areaColumns = { "areaMinX", "areaMinY", "areaMinZ", "areaMaxX", "areaMaxY", "areaMaxZ" };
            ...
            for (String col : finishAllColumns) {
                try {
                    stmt.executeUpdate("ALTER TABLE fr_tracks ADD COLUMN " + col + " REAL DEFAULT NULL");
                } catch (SQLException ignored) {}
            }

            plugin
                    .getDebugManager()
                    .logDatabaseOperation("[FormulaRacing] Tabelas do banco de dados verificadas com sucesso.");
        } catch (SQLException e) {
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("[FormulaRacing] Erro crítico ao criar tabelas: " + e.getMessage());
        }
    }
    ...
    public synchronized void normalizeAllTrackNames() {
        ...
        plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Iniciando normalização de trackNameWS...");
        plugin.getDebugManager().logDatabaseOperation("========================================");
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlSelect); ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    String newTrackNameWS = oldTrackNameWS.replaceAll("\\s+", "").toLowerCase();
                    ...
                    if (!oldTrackNameWS.equals(newTrackNameWS)) {
                        ...
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation(
                                        "[MIGRAÇÃO] Mapeado: '" + oldTrackNameWS + "' -> '" + newTrackNameWS + "'"
                                );
                    }
                }
            }
            ...
            if (trackNameMapping.isEmpty()) {
                plugin
                        .getDebugManager()
                        .logDatabaseOperation("[MIGRAÇÃO] Nenhuma pista precisa ser normalizada. Tudo OK!");
                plugin.getDebugManager().logDatabaseOperation("========================================");
                return;
            }
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("[MIGRAÇÃO] Encontradas " + trackNameMapping.size() + " pistas para normalizar");
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_tracks");
            String sqlUpdateTracks = "UPDATE fr_tracks SET trackNameWS = ? WHERE trackNameWS = ?";
            try (PreparedStatement ps = conn.prepareStatement(sqlUpdateTracks)) {
                for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
            ...
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_player_times");
            String sqlUpdatePlayerTimes = "UPDATE fr_player_times SET trackNameWS = ? WHERE trackNameWS = ?";
            try (PreparedStatement ps = conn.prepareStatement(sqlUpdatePlayerTimes)) {
                for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
            ...
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_checkpoint");
            String sqlUpdateCheckpoint = "UPDATE fr_checkpoint SET trackNameWS = ? WHERE trackNameWS = ?";
            try (PreparedStatement ps = conn.prepareStatement(sqlUpdateCheckpoint)) {
                for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
                    ps.setString(1, entry.getValue());
                    ps.setString(2, entry.getKey());
                    int updated = ps.executeUpdate();
                    if (updated > 0) {
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation(
                                        "[MIGRAÇÃO]   ✅ fr_checkpoint: " + updated + " checkpoints atualizados"
                                );
                    }
                }
            }
            ...
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_checkpoint_times");
            String sqlUpdateCheckpointTimes = "UPDATE fr_checkpoint_times SET trackNameWS = ? WHERE trackNameWS = ?";
            try (PreparedStatement ps = conn.prepareStatement(sqlUpdateCheckpointTimes)) {
                for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
                    ps.setString(1, entry.getValue());
                    ps.setString(2, entry.getKey());
                    int updated = ps.executeUpdate();
                    if (updated > 0) {
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation(
                                        "[MIGRAÇÃO]   ✅ fr_checkpoint_times: " + updated + " tempos de checkpoint atualizados"
                                );
                    }
                }
            }
            ...
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_boatutils");
            String sqlUpdateBoatUtils = "UPDATE fr_boatutils SET trackNameWS = ? WHERE trackNameWS = ?";
            try (PreparedStatement ps = conn.prepareStatement(sqlUpdateBoatUtils)) {
                for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
                    ps.setString(1, entry.getValue());
                    ps.setString(2, entry.getKey());
                    int updated = ps.executeUpdate();
                    if (updated > 0) {
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation(
                                        "[MIGRAÇÃO]   ✅ fr_boatutils: " + updated + " configurações atualizadas"
                                );
                    }
                }
            }
            ...
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_grid_positions");
            String sqlUpdateGrid = "UPDATE fr_grid_positions SET trackNameWS = ? WHERE trackNameWS = ?";
                ...
                for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
                    ...
                    if (updated > 0) {
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation(
                                        "[MIGRAÇÃO]   ✅ fr_grid_positions: " + updated + " posições de grid atualizadas"
                                );
                    }
                }
            }
            ...
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_cameras");
            String sqlUpdateCameras = "UPDATE fr_cameras SET trackNameWS = ? WHERE trackNameWS = ?";
            try (PreparedStatement ps = conn.prepareStatement(sqlUpdateCameras)) {
                for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
                    ps.setString(1, entry.getValue());
                    ps.setString(2, entry.getKey());
                    int updated = ps.executeUpdate();
                    if (updated > 0) {
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation("[MIGRAÇÃO]   ✅ fr_cameras: " + updated + " câmeras atualizadas");
                    }
                }
            }

            // ========== PASSO 8.5: Atualizar fr_track_finish_positions ==========
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_track_finish_positions");
            String sqlUpdateFinishPos = "UPDATE fr_track_finish_positions SET trackNameWS = ? WHERE trackNameWS = ?";
            try (PreparedStatement ps = conn.prepareStatement(sqlUpdateFinishPos)) {
                for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
                    ps.setString(1, entry.getValue());
                    ps.setString(2, entry.getKey());
                    int updated = ps.executeUpdate();
                    if (updated > 0) {
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation(
                                        "[MIGRAÇÃO]   ✅ fr_track_finish_positions: " + updated + " posições atualizadas"
                                );
                    }
                }
            }
            ...
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_timetrial_duels");
            String sqlUpdateDuels = "UPDATE fr_timetrial_duels SET trackNameWS = ? WHERE trackNameWS = ?";
                ...
                for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
                    ...
                    if (updated > 0) {
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation(
                                        "[MIGRAÇÃO]   ✅ fr_timetrial_duels: " + updated + " duelos atualizados"
                                );
                    }
                }
            }
            ...
            try {
                plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Atualizando tabela: fr_events");
                String sqlUpdateEvents = "UPDATE fr_events SET trackNameWS = ? WHERE trackNameWS = ?";
                try (PreparedStatement ps = conn.prepareStatement(sqlUpdateEvents)) {
                    for (Map.Entry<String, String> entry : trackNameMapping.entrySet()) {
                        ps.setString(1, entry.getValue());
                        ps.setString(2, entry.getKey());
                        int updated = ps.executeUpdate();
                        if (updated > 0) {
                            plugin
                                    .getDebugManager()
                                    .logDatabaseOperation("[MIGRAÇÃO]   ✅ fr_events: " + updated + " eventos atualizados");
                        }
                    }
                }
            } catch (SQLException e) {
                ...
                plugin
                        .getDebugManager()
                        .logDatabaseOperation("[MIGRAÇÃO]   ⚠️ fr_events: tabela não tem trackNameWS (OK)");
            }

            plugin.getDebugManager().logDatabaseOperation("========================================");
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] ✅ CONCLUÍDA COM SUCESSO!");
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("[MIGRAÇÃO] Total de pistas normalizadas: " + trackNameMapping.size());
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] Todas as tabelas foram atualizadas!");
            plugin.getDebugManager().logDatabaseOperation("========================================");
        } catch (SQLException e) {
            plugin.getDebugManager().logDatabaseOperation("[MIGRAÇÃO] ❌ ERRO CRÍTICO: " + e.getMessage());
            handleSqlError(e);
        }
    }
    ...
    public String getTrackNameFromDuelId(int duelId) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("[DB] Erro ao buscar track do duelo " + duelId + ": " + e.getMessage());
        }
        return null;
    }
...
======================================================= */
    ...
    public synchronized void setSelectedHeat(UUID uuid, Integer heatId) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao definir heat selecionado para " + uuid + ": " + e.getMessage());
        }
    }
        ...
        synchronized (this) {
            ...
            try {
                ...
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        if (rs.next()) {
                            boolean value = rs.getInt("timetrialScoreboard") == 1;
...
======================================================= */
    ...
    public synchronized UUID getPlayerUUIDByName(String playerName) {
        String sql = "SELECT uuid FROM fr_players WHERE LOWER(displayName) = LOWER(?) LIMIT 1";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public int getCheckpointCount(String trackNameWS) {
        ...
        synchronized (this) {
            ...
            String sql = "SELECT COUNT(*) AS total FROM fr_checkpoint WHERE LOWER(trackNameWS) = LOWER(?)";
            try {
                Connection conn = getOrConnect();
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
...
======================================================= */
    ...
    public synchronized boolean removeCheckpoint(String trackName, int checkpointId) {
        String sql = "DELETE FROM fr_checkpoint WHERE LOWER(trackNameWS) = LOWER(?) AND checkpointId = ?";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public synchronized boolean updateRegionBounds(
        ...
        String sql = "UPDATE fr_regions SET min_x=?, min_y=?, min_z=?, max_x=?, max_y=?, max_z=? WHERE id=?";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
        ...
        try {
            ...
            if (plugin.getDebugManager().isTimeTrialSystemEnabled()) {
                if (checkpointTimes.isEmpty()) {
                    ...
                    try (PreparedStatement psDebug = conn.prepareStatement(debugSql)) {
                        ...
                        try (ResultSet rsDebug = psDebug.executeQuery()) {
                            if (rsDebug.next()) {
                                ...
                                plugin
                                        .getDebugManager()
                                        .logTimeTrialSystem("§e[DB] Nenhum PB registrado - primeira volta");
                            }
                        }
                    }
                } else {
                    plugin
                            .getDebugManager()
                            .logTimeTrialSystem(
                                    "§a[DB] Carregados " + checkpointTimes.size() + " checkpoints para comparação"
                            );
                }
            }
        } catch (SQLException e) {
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("[FormulaRacing] Erro ao buscar checkpoints: " + e.getMessage());
            handleSqlError(e);
        }
...
======================================================= */
    ...
    public synchronized Location deleteTrack(String trackName) {
        ...
        String selectHologramSql = "SELECT world, x, y, z FROM fr_holograms WHERE LOWER(trackNameWS) = LOWER(?)";
        String[] deleteSqls = {
        ...
        try {
            ...
            plugin.getDebugManager().logDatabaseOperation("Pista '" + trackName + "' deletada completamente.");
        } catch (SQLException e) {
            try {
    ...
    public synchronized List<String> getAllTracks() {
        ...
        try {
            ...
            try (
                    PreparedStatement ps = conn.prepareStatement("SELECT trackName FROM fr_tracks");
    ...
    public synchronized boolean isTrackExists(String trackName) {
        ...
        String sql = "SELECT 1 FROM fr_tracks WHERE LOWER(trackNameWS) = LOWER(?) LIMIT 1";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, trackName.replaceAll("\\s+", ""));
...
======================================================= */
    ...
    public synchronized boolean setTrackIcon(String trackName, String iconName) {
        String sql = "UPDATE fr_tracks SET icon_name = ? WHERE LOWER(trackNameWS) = LOWER(?)";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public synchronized boolean removeGridPosition(String trackName, int gridNumber) {
        String sql = "DELETE FROM fr_grid_positions WHERE LOWER(trackNameWS) = LOWER(?) AND positionIndex = ?";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, trackName.replaceAll("\\s+", ""));
    ...
    public synchronized boolean clearGridPositions(String trackName) {
        String sql = "DELETE FROM fr_grid_positions WHERE LOWER(trackNameWS) = LOWER(?)";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, trackName.replaceAll("\\s+", ""));
    ...
    public synchronized List<Integer> getCamerasForTrack(String trackName) {
        ...
        String sql = "SELECT id FROM fr_cameras WHERE LOWER(trackNameWS) = LOWER(?) ORDER BY id ASC";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, trackName.replaceAll("\\s+", ""));
...
======================================================= */
    ...
    public synchronized String getTrackOwner(String trackNameWS) {
        String sql = "SELECT creatorName FROM fr_tracks WHERE LOWER(trackNameWS) = LOWER(?)";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public synchronized boolean setTrackOwner(String trackNameWS, String newOwnerName) {
        ...
        String sql = "UPDATE fr_tracks SET creatorName = ? WHERE REPLACE(LOWER(trackNameWS), ' ', '') = LOWER(?)";
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                String cleanedTrackName = trackNameWS.replace(" ", "").toLowerCase(Locale.ROOT);
                ...
                if (affectedRows > 0) {
                    ...
                    plugin
                            .getLogger()
                            .warning(
                                    "[SQLite] Pista '" + trackNameWS + "' não encontrada (mesmo ignorando espaços/letras)."
                            );
                    return false;
                }
            }
    ...
    public synchronized Map<String, TrackData> getAllTracksWithData() {
                ...
                while (rs.next()) {
                    ...
                    trackDataMap.put(
                            ...
                            new TrackData(
                                    ...
                                    getCheckpointCount(trackNameWS.replaceAll("\\s+", "").toLowerCase()) // Normaliza ao buscar checkpoints
                            )
                    );
                }
            }
        ...
        plugin
                .getDebugManager()
                .logDatabaseOperation("[getAllTracksWithData] Carregou " + trackDataMap.size() + " pistas");
        return trackDataMap;
    }
        ...
        try {
            ...
            if (plugin.getDebugManager().isTimeTrialSystemEnabled()) {
                if (newCheckpoints == null) {
                    plugin
                            .getDebugManager()
                            .logTimeTrialSystem("§c[DB] getTempCheckpoints retornou NULL para " + trackNameWS);
                } else if (newCheckpoints.isEmpty()) {
                    plugin
                            .getDebugManager()
                            .logTimeTrialSystem("§c[DB] getTempCheckpoints retornou VAZIO para " + trackNameWS);
                } else {
                    plugin
                            .getDebugManager()
                            .logTimeTrialSystem("§e[DB] Total de checkpoints temporários: " + newCheckpoints.size());
                }
            }
            ...
            List<TimerUtils.CheckpointData> filteredCheckpoints = new ArrayList<>();
            for (TimerUtils.CheckpointData cp : newCheckpoints) {
                if (cp.getTrack().replace(" ", "").equalsIgnoreCase(trackNameWS)) {
            ...
            if (plugin.getDebugManager().isTimeTrialSystemEnabled()) {
                ...
                for (TimerUtils.CheckpointData cp : filteredCheckpoints) {
                    plugin
                            .getDebugManager()
                            .logTimeTrialSystem("  → CP " + cp.getId() + " @ " + String.format("%.3f", cp.getTime()) + "s");
                }
            }
                    ...
                    (oldTimetrialId == null) ||
                            ...
                            (!oldFinished &&
                                    ...
                                    (newCheckpointsCount > oldCheckpoints ||
                                            (newCheckpointsCount == oldCheckpoints && roundedNewTime <= oldTime)));

            if (plugin.getDebugManager().isTimeTrialSystemEnabled()) {
                if (newIsBetter) {
                    plugin.getDebugManager().logTimeTrialSystem("§a[DB] Novo tempo É MELHOR → Salvando checkpoints!");
                } else {
                    plugin
                            .getDebugManager()
                            .logTimeTrialSystem(
            ...
            if (oldTimetrialId != null) {
                String sqlDeleteOld = "DELETE FROM fr_checkpoint_times WHERE timetrial_id = ?";
                try (PreparedStatement psDel = conn.prepareStatement(sqlDeleteOld)) {
                    ...
                    plugin
                            .getDebugManager()
                            .logTimeTrialSystem(
                                    "⚠️ Falha ao deletar checkpoints antigos (id=" + oldTimetrialId + "): " + e.getMessage()
                            );
                }
            }
            ...
            try {
                ...
                if (e.getMessage().contains("CONSTRAINT") || e.getMessage().contains("UNIQUE")) {
                    ...
                    executeCheckpointBatch(conn, sqlInsert, newCheckpoints, playerUUID, trackNameWS, newTimetrialId);
                    clearCheckpointsCache(trackNameWS); // Added
                    plugin.getDebugManager().logDatabaseOperation("[FormulaRacing] Recuperação concluída com sucesso.");
                } else {
    ...
    private void executeCheckpointBatch(
        ...
        if (plugin.getDebugManager().isTimeTrialSystemEnabled()) {
            plugin
                    .getDebugManager()
                    .logTimeTrialSystem(
                            "§e[DB BATCH] Inserindo " + checkpoints.size() + " checkpoints para timetrial_id=" + tid
                    );
        }
        ...
        try (PreparedStatement psIns = conn.prepareStatement(sql)) {
            ...
            if (plugin.getDebugManager().isTimeTrialSystemEnabled()) {
                ...
                plugin
                        .getDebugManager()
                        .logTimeTrialSystem(
                                "§a[DB BATCH] Batch executado: " + successCount + "/" + count + " inserções bem-sucedidas"
                        );
            }
        }
    }
    ...
    public synchronized void cleanOrphanedCheckpoints() {
        String sql = "DELETE FROM fr_checkpoint_times WHERE timetrial_id NOT IN (SELECT id FROM fr_player_times)";
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                if (deleted > 0) {
                    plugin
                            .getDebugManager()
                            .logDatabaseOperation(
                                    "[FormulaRacing] 🧹 Limpeza de checkpoints órfãos: " + deleted + " registros removidos."
                            );
                }
            }
        } catch (SQLException e) {
            plugin.getDebugManager().logDatabaseOperation("⚠️ Erro ao limpar checkpoints órfãos: " + e.getMessage());
            handleSqlError(e);
        }
    }
        ...
        try {
                ...
                try (ResultSet rsOwn = psOwn.executeQuery()) {
                    if (rsOwn.next()) {
                        ...
                        plugin
                                .getDebugManager()
                                .logTimeTrialSystem(
                                        "[saveFullTime] Melhor tempo anterior do jogador: " + playerOwnBestTime
                                );
                    }
                }
            }
            ...
            String ownBestWithCheckpointsSql =
                    "SELECT pt.bestTime " +
                            ...
                            "GROUP BY pt.id " +
                            "ORDER BY pt.bestTime ASC LIMIT 1";
            try (PreparedStatement psCheckpoints = conn.prepareStatement(ownBestWithCheckpointsSql)) {
                ...
                try (ResultSet rsCheckpoints = psCheckpoints.executeQuery()) {
                    if (rsCheckpoints.next()) {
                        playerBestTimeWithCheckpoints = rsCheckpoints.getDouble("bestTime");
                        plugin
                                .getDebugManager()
                                .logTimeTrialSystem(
            ...
            boolean shouldSaveCheckpoints = roundedTime < playerBestTimeWithCheckpoints;
            ...
            if (shouldSaveTime) {
                        ...
                        if (rs.next()) {
                            ...
                            if (roundedTime < (prevBestTime - 0.001)) {
                                ...
                                if (prevBestPlayer.equals(playerName)) {
                                    ...
                                    plugin
                                            .getDebugManager()
                                            .logTimeTrialSystem("[saveFullTime] ✅ Jogador melhorou seu PRÓPRIO recorde!");
                                } else {
                            ...
                            plugin.getDebugManager().logTimeTrialSystem("[saveFullTime] ✅ PRIMEIRO RECORDE da pista!");
                            isNewGlobalRecord = true;
                        }
                    }
                }
                ...
                if (isNewGlobalRecord) {
                    plugin
                            .getDebugManager()
                            .logTimeTrialSystem("[saveFullTime] 📢 Enviando mensagem de recorde para o Discord!");
                    ...
                    try (
                        ...
                        try (ResultSet rsName = psName.executeQuery()) {
                            if (rsName.next()) {
                                displayTrackName = rsName.getString("trackName");
                            }
                        }
                    } catch (SQLException ignored) {
                    ...
                    plugin
                            .getDebugManager()
                            .logTimeTrialSystem("[saveFullTime] Não enviando para Discord (não é recorde global)");
                }
            }
            ...
            try {
                ...
                plugin.getDebugManager().logDatabaseOperation("❌ Erro na transação de FullTime: " + e.getMessage());
            } finally {
    ...
    public synchronized void savePartialTime(
        ...
        try {
            ...
            plugin.getDebugManager().logDatabaseOperation("❌ Erro ao salvar tempo parcial: " + e.getMessage());
    ...
    public synchronized List<Location> getCameras(String trackName) {
        ...
        // ✅ Remove apenas espaços, sem toLowerCase
        String trackNameWS = trackName.replaceAll("\\s+", "");
        String sql = "SELECT x, y, z FROM fr_cameras WHERE LOWER(trackNameWS) = LOWER(?)";
        ...
        try {
            ...
            plugin.getDebugManager().logDatabaseOperation("Erro ao listar câmeras: " + e.getMessage());
            handleSqlError(e);
        }
        return cameras;
    }
    ...
    public synchronized void setSelectedEvent(UUID uuid, String eventName) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("[FormulaRacing] Erro ao definir selectedEvent: " + e.getMessage());
            handleSqlError(e);
        }
    }
    ...
    public synchronized List<TrackRecord> getTopTimes(String trackName) {
        ...
        try {
            ...
            plugin.getDebugManager().logDatabaseOperation("Erro ao carregar top times: " + e.getMessage());
            handleSqlError(e);
        }
        return topTimes;
    }
    ...
    public synchronized TrackData getTrackData(String trackName) {
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlExact)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        return createTrackDataFromResultSet(rs, rs.getString("trackNameWS"));
                    }
                }
            }
    ...
    public synchronized Location getTrackSpawn(String trackName) {
        if (trackName == null || trackName.isEmpty()) {
            plugin.getDebugManager().logDatabaseOperation("[getTrackSpawn] trackName é null ou vazio!");
            return null;
        }
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                    ...
                    if (rs.next()) {
                        ...
                        if (world == null) {
                            plugin
                                    .getDebugManager()
                                    .logDatabaseOperation(
                                            "[getTrackSpawn] Mundo '" + worldName + "' não encontrado no servidor!"
                                    );
                            return null;
                        }
                        ...
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation(
                                        "[getTrackSpawn] Nenhuma pista encontrada com trackNameWS = '" + trackNameWS + "'"
                                );
                    }
                }
            } catch (SQLException e) {
                plugin
                        .getDebugManager()
                        .logDatabaseOperation("[getTrackSpawn] Erro ao buscar spawn: " + e.getMessage());
                handleSqlError(e);
            }
        } catch (SQLException e) {
            plugin.getDebugManager().logDatabaseOperation("[getTrackSpawn] Erro na conexão: " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
...
======================================================= */
    ...
    public synchronized void resetPlayerTimes(String playerUUID, String trackNameWS) {
        ...
        String sqlPlayerTimes = "DELETE FROM fr_player_times WHERE LOWER(trackNameWS) = LOWER(?) AND player_uuid = ?";
        ...
        try {
            ...
            plugin.getDebugManager().logDatabaseOperation("[FormulaRacing] Erro ao resetar tempos: " + e.getMessage());
            handleSqlError(e);
        }
    }
    ...
    public synchronized boolean deletePlayerBestTimeOnTrack(String trackName, String playerName) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("[FormulaRacing] Erro ao deletar melhor tempo: " + e.getMessage());
            handleSqlError(e);
            return false;
        }
    }
    ...
    public synchronized boolean resetAllTrackTimes(String trackNameWS) {
        String trackNormalized = trackNameWS.replaceAll("\\s+", "");
        String deletePlayerTimesSql = "DELETE FROM fr_player_times WHERE LOWER(trackNameWS) = LOWER(?)";
        String sqlCheckpointTimes = "DELETE FROM fr_checkpoint_times WHERE LOWER(trackNameWS) = LOWER(?)";
...
======================================================= */
    ...
    public synchronized boolean saveHologramLocation(String trackName, Location location) {
        ...
        String trackNameWS = trackName.replaceAll("\\s+", "");
        String sqlSelect = "SELECT x, y, z, world FROM fr_holograms WHERE LOWER(trackNameWS) = LOWER(?)";
        ...
        try {
            ...
            try (PreparedStatement psUpdate = conn.prepareStatement(sqlInsertOrUpdate)) {
                ...
                if (rows > 0) {
                    plugin
                            .getDebugManager()
                            .logDatabaseOperation(
                                    "[FormulaRacing] Holograma da pista '" + trackName + "' salvo/atualizado com sucesso."
                            );
                    return true;
                }
            }
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("[FormulaRacing] Erro ao salvar holograma: " + e.getMessage());
            handleSqlError(e);
        }
        return false;
    }
    ...
    public synchronized String getIcon(String trackName) {
        ...
        String sql = "SELECT icon_name FROM fr_tracks WHERE LOWER(trackNameWS) = LOWER(?)";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            plugin.getDebugManager().logDatabaseOperation("Erro ao buscar ícone: " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized Location getHologramLocation(String trackName) {
        ...
        String trackNameWS = trackName.replaceAll("\\s+", "");
        String sql = "SELECT world, x, y, z FROM fr_holograms WHERE LOWER(trackNameWS) = LOWER(?)";
...
======================================================= */
    ...
    public synchronized void setTurningForwardAcceleration(String trackNameWS, double value) {
        ...
        try {
            ...
            plugin.getDebugManager().logDatabaseOperation("Database error: " + e.getMessage());
            if (e instanceof SQLException) handleSqlError((SQLException) e);
        }
    }
...
======================================================= */
    ...
    public synchronized Double getGravity(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar gravidade para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized Double getYawAcceleration(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar aceleração de yaw para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized Double getForwardAcceleration(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar aceleração frontal para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
...
======================================================= */
    ...
    public synchronized Double getBackwardAcceleration(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar aceleração traseira para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized Double getTurningForwardAcceleration(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation(
                            "Erro ao buscar aceleração de giro frontal para " + trackNameWS + ": " + e.getMessage()
                    );
            handleSqlError(e);
        }
        return null;
    ...
    public synchronized Boolean getAllowAccelerationStacking(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation(
                            "Erro ao buscar empilhamento de aceleração para " + trackNameWS + ": " + e.getMessage()
                    );
            handleSqlError(e);
        }
        return null;
    ...
    public synchronized Boolean getUnderwaterControl(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation(
                            "Erro ao buscar controle subaquático para " + trackNameWS + ": " + e.getMessage()
                    );
            handleSqlError(e);
        }
        return null;
    ...
    public synchronized Integer getCoyoteTime(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar coyote time para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized Boolean getWaterJumping(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar pulo na água para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized Double getSwimForce(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar força de nado para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized List<Player> getPlayersInDuel(int duelId) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("Erro ao buscar jogadores do duelo " + duelId + ": " + e.getMessage());
            handleSqlError(e);
        }
        return players;
    }
...
======================================================= */
    ...
    public synchronized void createDuel(
        ...
        try {
            ...
            plugin.getDebugManager().logDuelSystem("[FormulaRacing] Erro ao registrar duelo: " + e.getMessage());
            handleSqlError(e);
        }
    }
    ...
    public synchronized Integer getCollisionMode(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar modo de colisão para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized Boolean getAirStepping(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar air stepping para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized Boolean getTenStepInterpolation(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation(
                            "Erro ao buscar interpolação de 10 passos para " + trackNameWS + ": " + e.getMessage()
                    );
            handleSqlError(e);
        }
        return null;
    ...
    public synchronized Integer getCollisionResolution(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation(
                            "Erro ao buscar resolução de colisão para " + trackNameWS + ": " + e.getMessage()
                    );
            handleSqlError(e);
        }
        return null;
    ...
    public synchronized String getPerBlockSetting(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation(
                            "Erro ao buscar configuração por bloco para " + trackNameWS + ": " + e.getMessage()
                    );
            handleSqlError(e);
        }
        return null;
    ...
    public synchronized Boolean getSurfaceWaterControl(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation(
                            "Erro ao buscar controle de água na superfície para " + trackNameWS + ": " + e.getMessage()
                    );
            handleSqlError(e);
        }
        return null;
    ...
    public synchronized Integer getPlayerTimeId(String trackNameWS, UUID playerUUID, double bestTime)
            throws SQLException {
        String sql =
    ...
    public synchronized boolean isBoatUtilsExclusive(String trackNameWS) {
        ...
        final String sql = "SELECT exclusiveMode FROM fr_boatutils WHERE LOWER(trackNameWS) = LOWER(?)";
    ...
    public synchronized Map<String, Object> getBoatUtilsRaw(String trackNameWS) {
        ...
        String trackNameNormalized = trackNameWS.replaceAll("\\s+", "");
        final String sql = "SELECT * FROM fr_boatutils WHERE LOWER(trackNameWS) = LOWER(?)";
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation("Erro ao buscar boat utils brutos para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized void setPerBlockSetting(String trackNameWS, String value) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation(
                            "Erro ao definir configuração por bloco para " + trackNameWS + ": " + e.getMessage()
                    );
            if (e instanceof SQLException) handleSqlError((SQLException) e);
        }
    }
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    ...
                    if (rs.getFloat("defaultSlipperiness") != BoatUtilsVanillaValues.DEFAULT_SLIPPERINESS) modified =
                            true;
                    ...
                    if (
                            rs.getFloat("turningForwardAcceleration") != BoatUtilsVanillaValues.TURN_FORWARD_ACCEL
                    ) modified = true;
                    if (
                            rs.getBoolean("allowAccelerationStacking") != BoatUtilsVanillaValues.ALLOW_ACCEL_STACKING
                    ) modified = true;
                    if (rs.getBoolean("underwaterControl") != BoatUtilsVanillaValues.UNDERWATER_CONTROL) modified =
                            true;
                    if (rs.getBoolean("surfaceWaterControl") != BoatUtilsVanillaValues.SURFACE_WATER_CONTROL) modified =
                            true;
                    ...
                    if (
                            rs.getBoolean("tenStepInterpolation") != BoatUtilsVanillaValues.TEN_STEP_INTERPOLATION
                    ) modified = true;
                    if (rs.getInt("collisionResolution") != BoatUtilsVanillaValues.COLLISION_RESOLUTION) modified =
                            true;

                    if (
                            !Objects.equals(rs.getString("customSlipperiness"), BoatUtilsVanillaValues.CUSTOM_SLIPPERINESS)
                    ) modified = true;
                    if (
                            !Objects.equals(rs.getString("perBlockSetting"), BoatUtilsVanillaValues.PER_BLOCK_SETTING)
                    ) modified = true;
            ...
            plugin
                    .getDebugManager()
                    .logDatabaseOperation(
                            "Erro ao verificar se pista " + trackNameWS + " tem boat utils: " + e.getMessage()
                    );
            handleSqlError(e);
        }
        return false;
    ...
    public synchronized List<UUID> getActivePlayersInDuel(int duelId) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("Erro ao buscar jogadores ativos no duelo " + duelId + ": " + e.getMessage());
            handleSqlError(e);
        }
    ...
    public synchronized int getActiveDuelId(UUID uuid) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("Falha ao buscar Duelo Ativo para " + uuidTarget + ": " + e.getMessage());
            handleSqlError(e);
        }
        return -1;
    }
    ...
    public synchronized Map<String, Float> getCustomSlipperiness(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logBoatUtils("Erro ao buscar fricção customizada para " + trackNameWS + ": " + e.getMessage());
            if (e instanceof SQLException) handleSqlError((SQLException) e);
        }
        return result;
    }
    ...
    public void setTrackFinishPos(String trackNameWS, int position, Location loc) {
        String sql =
    ...
    private synchronized void saveCustomSlipperiness(String trackNameWS, Map<String, Float> map) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logBoatUtils("Erro ao salvar fricção customizada para " + trackNameWS + ": " + e.getMessage());
            if (e instanceof SQLException) handleSqlError((SQLException) e);
        }
    }
    ...
    public synchronized void setAirControl(String trackNameWS, boolean value) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logBoatUtils("Erro ao definir controle aéreo para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
    }
    ...
    public synchronized Boolean getAirControl(String trackNameWS) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logBoatUtils("Erro ao buscar controle aéreo para " + trackNameWS + ": " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
     ...
     */
    public synchronized Optional<dev.EfraGroup.formulaRacing.Event.Events> getPlayerSelectedEvent(UUID playerUUID) {
        ...
        try {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        if (plugin.getRaceEventManager() != null) {
                            Optional<dev.EfraGroup.formulaRacing.Event.Events> eventOpt = plugin
                                    .getRaceEventManager()
                                ...
                                plugin
                                        ...
                                        .logDatabaseOperation(
                                                "[DB DEBUG] Evento encontrado na memória: " +
                                                        ...
                                                        " (EventObj: " +
                                                        System.identityHashCode(eventOpt.get()) +
                                                        ")"
                                        );
                                return eventOpt;
                            }
                        ...
                        plugin
                                .getDebugManager()
                                .logDatabaseOperation(
                                        "[DB DEBUG] Evento ID " + eventId + " não encontrado na memória, carregando do banco..."
                                );
                        dev.EfraGroup.formulaRacing.Database.EventsDatabaseManager eventsDb =
                                new dev.EfraGroup.formulaRacing.Database.EventsDatabaseManager(this, plugin);
            ...
            if (e.getMessage().contains("no such column") || e.getMessage().contains("selected_event_id")) {
                try {
                    Connection conn = getOrConnect();
                    try (
                            PreparedStatement ps = conn.prepareStatement(
        ...
        if (plugin.getQuickRaceManager() != null && plugin.getQuickRaceManager().isQuickRaceActive()) {
            Optional<dev.EfraGroup.formulaRacing.Event.Events> quickEvent = plugin
                    .getQuickRaceManager()
                    .getCurrentQuickRace();
            if (quickEvent.isPresent()) {
                ...
                if (heat != null) {
                    ...
                    if (player != null && player.hasPermission("formularacing.event.admin")) {
                        return quickEvent;
                    }
                }
            }
     ...
     */
    public synchronized void setPlayerSelectedEvent(UUID playerUUID, dev.EfraGroup.formulaRacing.Event.Events event) {
        String sql = "UPDATE fr_players SET selected_event_id = ? WHERE uuid = ?";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public synchronized void clearPlayerSelectedEvent(UUID playerUUID) {
        String sql = "UPDATE fr_players SET selected_event_id = NULL WHERE uuid = ?";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, playerUUID.toString());
    ...
    public synchronized void clearAllPlayerSelectedEvents() {
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                plugin
                        .getDebugManager()
                        .logDatabaseOperation("[FormulaRacing] Limpou eventos selecionados de " + updated + " jogadores");
            }
        } catch (SQLException e) {
            // Se a coluna não existe, não faz nada
            if (!e.getMessage().contains("no such column") && !e.getMessage().contains("selected_event_id")) {
    ...
    public synchronized Object[] getPlayerBestTimeOnDuel(UUID uuid, int duelId) {
        ...
        try {
            ...
            plugin.getDebugManager().logDuelSystem("[FormulaRacing] Erro ao buscar Duel Time: " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized void saveDuelLapTime(
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("[FormulaRacing] Erro ao salvar tempo de volta do duelo: " + e.getMessage());
            handleSqlError(e);
        }
    }
    ...
    public synchronized void saveDuelFinalTime(
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("[FormulaRacing] Erro ao salvar tempo final do duelo: " + e.getMessage());
            handleSqlError(e);
        }
    }
    ...
    public synchronized Double getPlayerBestLapTimeInDuel(UUID playerUUID, int duelId) {
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("[FormulaRacing] Erro ao buscar melhor volta no duelo: " + e.getMessage());
            handleSqlError(e);
        }
        return null;
    }
    ...
    public synchronized void saveDuelCheckpointTime(
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("[FormulaRacing] Erro ao salvar checkpoint do duelo: " + e.getMessage());
            handleSqlError(e);
        }
    }
     ...
     */
    public synchronized Map<Integer, Double> getDuelCheckpointTimes(UUID playerUUID, int duelId) {
        Map<Integer, Double> checkpointTimes = new HashMap<>();
        String sql =
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("[FormulaRacing] Erro ao buscar checkpoints do duelo: " + e.getMessage());
            handleSqlError(e);
        }
        return checkpointTimes;
    }
     ...
     */
    public synchronized void clearDuelCheckpointTimes(UUID playerUUID, int duelId) {
        String sql = "DELETE FROM fr_timetrial_duels_checkpoint_times WHERE player_uuid = ? AND duel_id = ?";
        ...
        try {
            ...
            plugin
                    .getDebugManager()
                    .logDuelSystem("[FormulaRacing] Erro ao limpar checkpoints do duelo: " + e.getMessage());
            handleSqlError(e);
        }
    }
    ...
    private void handleSqlError(SQLException e) {
        ...
        plugin.getLogger().log(Level.SEVERE, "[FormulaRacing] Erro SQL detectado!", e);
        ...
        if (connectionIssue) {
            plugin.getLogger().warning("[FormulaRacing] Conexão com o banco perdida. Tentando resetar...");
            try {
    ...
    public class PitStopData {
        ...
        public PitStopData(String trackName, Location entryMin, Location entryMax,
                           Location exitMin, Location exitMax, Location areaMin,
                           Location areaMax, Location startMin, Location startMax) {
            this.trackName = trackName;
        ...
        public Location getStartMin() { return startMin; }
        public Location getStartMax() { return startMax; }
     ...
     */
    public synchronized boolean savePitStopEntry(String trackName, Location min, Location max) {
     ...
     */
    public synchronized boolean savePitStopExit(String trackName, Location min, Location max) {
        if (trackName == null) return false;
        String trackNameWS = trackName.replaceAll("\\s+", "");
        String sql =
                "INSERT INTO fr_pit_stops (trackNameWS, " +
     ...
     */
    public synchronized boolean savePitStopArea(String trackName, Location min, Location max) {
        if (trackName == null) return false;
        String trackNameWS = trackName.replaceAll("\\s+", "");
        String sql =
                "INSERT INTO fr_pit_stops (trackNameWS, " +
    ...
    public synchronized boolean removePitStop(String trackName) {
        if (trackName == null) return false;
        String trackNameWS = trackName.replaceAll("\\s+", "");
        String sql = "DELETE FROM fr_pit_stops WHERE LOWER(trackNameWS) = LOWER(?)";
        try {
            Connection conn = getOrConnect();
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, trackNameWS);
     ...
     */
    private Location createLocation(World world, ResultSet rs, String prefix) throws SQLException {
    ...
    private TrackData createTrackDataFromResultSet(ResultSet rs, String trackNameWSQuery) throws SQLException {
    ...
    public void saveFullTimeAsync(
        ...
        org.bukkit.Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            saveFullTime(playerUUID, playerName, trackName, time, checkpointsReached);
        });
    }

    public CompletableFuture<Object[]> getPlayerBestTimeAsync(String playerName, String trackName) {
    ...
    public CompletableFuture<Integer> getPlayerRankAsync(UUID playerUUID, String trackNameWS) {
