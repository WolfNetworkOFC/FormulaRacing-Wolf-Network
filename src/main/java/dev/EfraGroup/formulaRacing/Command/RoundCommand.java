...
public class RoundCommand extends BaseCommand {
    ...
    public void onDefault(Player player) {
        ...
        if (eventOpt.isEmpty()) {
            player.sendMessage(String.valueOf(ChatColor.RED) + "✗ Nenhum evento selecionado! Use /event info <evento> primeiro.");
        } else {
            ...
            if (roundOpt.isPresent()) {
                ...
                player.sendMessage(var10001 + "Nenhuma rodada ativa no evento " + event.getDisplayName());
                ClickableMessageUtil.sendClickableLine(player, String.valueOf(ChatColor.GRAY) + "Use ", "/round create <tipo>", " para criar uma.", "/round create ", "§aClique para sugerir o comando de criação", true);
            }
    ...
    public void onCreate(Player player, Events event, RoundType type) {
        if (event == null) {
            event = this.database.getPlayerSelectedEvent(player.getUniqueId()).orElse(null);
        }
        ...
        if (event == null) {
            ...
            this.eventManager.createRound(event, type, nextIndex).thenAccept((r) -> {
                if (r != null) {
                    player.sendMessage(String.valueOf(ChatColor.GREEN) + "✓ Round " + nextIndex + " (" + String.valueOf(type) + ") criado com sucesso!");
                    if (type == RoundType.PRACTICE || type == RoundType.QUALIFICATION) {
                        this.eventManager.createHeat(r, 1).thenAccept((heat) -> {
                            if (heat != null) {
                                player.sendMessage(String.valueOf(ChatColor.GRAY) + "  ➜ Heat 1 criado automaticamente.");
                            }

                        });
                    }
                } else {
                    player.sendMessage(String.valueOf(ChatColor.RED) + "✗ Erro ao criar round (Falha no Banco de Dados).");
                }
    ...
    public void onInfo(Player player, Rounds round) {
        boolean isAdmin = player.hasPermission("formularacing.event.admin");
        player.sendMessage("");
        TextComponent header = new TextComponent("");
        header.addExtra(ClickableMessageUtil.getRefreshButton("/round info " + round.getId(), "Atualizar"));
        header.addExtra(new TextComponent(" "));
        TextComponent title = new TextComponent(("ROUND " + round.getRoundNumber()).toUpperCase());
        title.setColor(ChatColor.GOLD);
        title.setBold(true);
        header.addExtra(title);
        String var10003 = String.valueOf(ChatColor.GRAY);
        header.addExtra(new TextComponent(var10003 + " (" + round.getType().name() + ")"));
        header.addExtra(new TextComponent(" "));
        header.addExtra(ClickableMessageUtil.getButton("Event Info", ChatColor.BLUE, "/event info " + round.getEvent().getDisplayName(), "Voltar para o evento", Action.RUN_COMMAND));
        player.spigot().sendMessage(header);
        String var10001 = String.valueOf(ChatColor.YELLOW);
        player.sendMessage(var10001 + "  Estado: " + String.valueOf(ChatColor.WHITE) + String.valueOf(round.getState()));
        player.sendMessage("");
        String var10002 = String.valueOf(ChatColor.GOLD);
        TextComponent heatsHeader = new TextComponent(var10002 + String.valueOf(ChatColor.BOLD) + "  HEATS:");
        if (isAdmin) {
            heatsHeader.addExtra(new TextComponent("   "));
            heatsHeader.addExtra(ClickableMessageUtil.getButton("+ Novo Heat", ChatColor.GREEN, "/heat create " + round.getEvent().getDisplayName() + " " + round.getDisplayName(), "Adicionar Heat", Action.RUN_COMMAND));
        }

        player.spigot().sendMessage(heatsHeader);
        if (round.getHeats().isEmpty()) {
            player.sendMessage(String.valueOf(ChatColor.GRAY) + "   (Nenhum heat criado)");
        } else {
            for(Heats heat : round.getHeatsOrdered()) {
                ...
                TextComponent line = new TextComponent("   " + heatName + " - ");
                ...
                line.addExtra(ClickableMessageUtil.getButton("Info", ChatColor.AQUA, "/heat info " + heat.getId(), "Ver detalhes do heat", Action.RUN_COMMAND));
                if (isAdmin) {
                    line.addExtra(new TextComponent(" "));
                    line.addExtra(ClickableMessageUtil.getButton("✖", ChatColor.RED, "/heat delete " + heat.getId(), "Excluir heat", Action.SUGGEST_COMMAND));
                }
        ...
        if (round.getType() == RoundType.QUALIFICATION || round.getType() == RoundType.PRACTICE) {
            player.sendMessage("");
            ClickableMessageUtil.sendClickableLine(player, "  ", String.valueOf(ChatColor.GOLD) + "[VER RESULTADOS GERAIS]", "", "/round results " + round.getId(), "Ver ranking combinado", false);
        }
    ...
    public void onResults(Player player, Rounds round) {
        if (round == null) {
    ...
    private void displayRoundRanking(Player player, Rounds round) {
        ...
        player.sendMessage(var10001 + "    RESULTADOS: " + String.valueOf(ChatColor.WHITE) + round.getDisplayName());
        var10001 = String.valueOf(ChatColor.GOLD);
        player.sendMessage(var10001 + String.valueOf(ChatColor.BOLD) + "═══════════════════════════════");
        ...
        for(Heats heat : round.getHeats().values()) {
            for(Driver driver : heat.getDrivers().values()) {
                if (driver.getFastestLap() != null) {
                    ...
                    if (!bestLaps.containsKey(driver.getUuid()) || time < (Long)bestLaps.get(driver.getUuid())) {
        ...
        if (bestLaps.isEmpty()) {
            player.sendMessage(String.valueOf(ChatColor.GRAY) + "  Ainda não há tempos registrados.");
        } else {
            ...
            for(UUID uuid : sortedDrivers) {
                ...
                String time = ApiUtilities.formatRaceTime((Long)bestLaps.get(uuid));
                String color = pos <= 3 ? ChatColor.GREEN.toString() : ChatColor.GRAY.toString();
                if (pos == 1) {
                    color = ChatColor.GOLD.toString();
                }

                player.sendMessage(String.format("  " + color + "#%d " + String.valueOf(ChatColor.WHITE) + "%-12s " + String.valueOf(ChatColor.YELLOW) + "%s", pos, name, time));
                ++pos;
                if (pos > 15) {
                    break;
                }
    ...
    public void onStart(Player player, Rounds round) {
        if (round == null) {
        ...
        if (round == null) {
            ...
            if (round.start()) {
                ...
                player.sendMessage(var10001 + "✓ Round " + round.getDisplayName() + " iniciado!");
            } else {
                player.sendMessage(String.valueOf(ChatColor.RED) + "✗ Falha ao iniciar round.");
            }
    ...
    public void onFinish(Player player, Rounds round) {
        if (round == null) {
            Events event = this.database.getPlayerSelectedEvent(player.getUniqueId()).orElse(null);
            if (event != null) {
                round = event.getSchedule().getCurrentRound().orElse(null);
        ...
        if (round == null) {
            ...
            round.finish();
            String var10001 = String.valueOf(ChatColor.GREEN);
            player.sendMessage(var10001 + "✓ Round " + round.getDisplayName() + " finalizado!");
        }
    }
    ...
    public void onDelete(Player player, Rounds round) {
        if (this.plugin.getRaceEventManager().removeRound(round)) {
            String var10001 = String.valueOf(ChatColor.GREEN);
            player.sendMessage(var10001 + "✓ Round " + round.getDisplayName() + " removido.");
        } else {
            player.sendMessage(String.valueOf(ChatColor.RED) + "✗ Falha ao remover round.");
        }
    ...
    public void onClear(Player player,Rounds round) {
        if (round == null) {
            Events event = this.database.getPlayerSelectedEvent(player.getUniqueId()).orElse(null);
            if (event != null) {
                round = event.getSchedule().getCurrentRound().orElse(null);
        ...
        if (round == null) {
            ...
            for(Heats heat : round.getHeats().values()) {
                if (heat.getHeatState() != HeatState.SETUP) {
                    ...
                    player.sendMessage(var10001 + "✗ Não é possível remover pilotos do " + heat.getName() + " pois ele já foi iniciado ou finalizado.");
                    return;
                }
            }
            ...
            player.sendMessage(var8 + "✓ Todos os pilotos foram removidos do round " + round.getDisplayName());
        }
    }
    ...
    public void onFill(Player player, Rounds round, String sortMode, String groupMode) {
        if (round == null) {
            Events event = this.database.getPlayerSelectedEvent(player.getUniqueId()).orElse(null);
            if (event != null) {
                round = event.getSchedule().getCurrentRound().orElse(null);
        ...
        if (round == null) {
            ...
            if (event == null) {
                player.sendMessage(String.valueOf(ChatColor.RED) + "✗ Evento associado ao round não encontrado!");
            } else if (event.getState() != EventState.SETUP) {
                player.sendMessage(String.valueOf(ChatColor.RED) + "✗ Evento já começou! Não é possível preencher heats.");
            } else {
                List<Heats> heats = new ArrayList(round.getHeats().values());
                if (heats.isEmpty()) {
                    String var46 = String.valueOf(ChatColor.RED);
                    player.sendMessage(var46 + "✗ Nenhum heat criado na rodada R" + round.getRoundNumber() + "!");
                    ...
                    switch (groupMode.toLowerCase()) {
                        case "all":
                            playersToAdd.addAll(event.getSubscribers().keySet());
                            int remainingSlots = totalCapacity - subscriberCount;
                            if (remainingSlots > 0 && reserveCount > 0) {
                                List<UUID> reserves = new ArrayList(event.getReserves().keySet());
                                if (sortMode.equalsIgnoreCase("sorted")) {
                                    reserves = this.sortPlayersByRanking(reserves, event.getTrackNameWS());
                                } else {
                                    Collections.shuffle(reserves);
                                }

                                int reservesToAdd = Math.min(remainingSlots, reserveCount);

                                for(int i = 0; i < reservesToAdd; ++i) {
                                    playersToAdd.add((UUID)reserves.get(i));
                                }

                                for(int i = reservesToAdd; i < reserves.size(); ++i) {
                                    excludedPlayers.add((UUID)reserves.get(i));
                                }
                            } else {
                                excludedPlayers.addAll(event.getReserves().keySet());
                            }
                            break;
                        case "signed":
                            playersToAdd.addAll(event.getSubscribers().keySet());
                    ...
                    if (sortMode.equalsIgnoreCase("sorted")) {
                        playersToAdd = this.sortPlayersByRanking(playersToAdd, event.getTrackNameWS());
                    } else {
                    ...
                    for(Heats heat : heats) {
                        ...
                        String var10001 = String.valueOf(ChatColor.YELLOW);
                        player.sendMessage(var10001 + "⚙ Preenchendo " + heat.getName() + "...");

                        for(int i = 0; i < slotsAvailable && !playerQueue.isEmpty(); ++i) {
                    ...
                    if (!playerQueue.isEmpty() || !excludedPlayers.isEmpty()) {
                        player.sendMessage("");
                        player.sendMessage(String.valueOf(ChatColor.YELLOW) + "⚠ Jogadores que ficaram de fora:");

                        while(!playerQueue.isEmpty()) {
                            UUID uuid = (UUID)playerQueue.poll();
                            Player p = this.plugin.getServer().getPlayer(uuid);
                            String name = p != null ? p.getName() : this.plugin.getServer().getOfflinePlayer(uuid).getName();
                            String var40 = String.valueOf(ChatColor.GRAY);
                            player.sendMessage(var40 + "  - " + name + " (sem vaga)");
                        }
                        ...
                        for(UUID uuid : excludedPlayers) {
                            ...
                            player.sendMessage(var41 + "  - " + name + " (reserva)");
                        }
                    }

                    player.sendMessage("");
                    String var42 = String.valueOf(ChatColor.GREEN);
                    player.sendMessage(var42 + "✓ Heats preenchidos com sucesso!");
                    var42 = String.valueOf(ChatColor.GRAY);
                    player.sendMessage(var42 + "Modo: " + String.valueOf(ChatColor.WHITE) + (sortMode.equalsIgnoreCase("sorted") ? "Ordenado por ranking" : "Aleatório"));
                    var42 = String.valueOf(ChatColor.GRAY);
                    player.sendMessage(var42 + "Pilotos adicionados: " + String.valueOf(ChatColor.WHITE) + addedCount);
                    var42 = String.valueOf(ChatColor.GRAY);
                    player.sendMessage(var42 + "Capacidade total: " + String.valueOf(ChatColor.WHITE) + totalCapacity + " vagas em " + heats.size() + " heat(s)");
                }
            }
        }
    }

    private List<UUID> sortPlayersByRanking(List<UUID> players, String trackName) {
        if (trackName != null && !trackName.isEmpty()) {
            Map<UUID, Long> playerTimes = new HashMap();

            for(UUID uuid : players) {
                String sql = "SELECT bestTime FROM fr_player_times WHERE uuid = ? AND LOWER(trackNameWS) = LOWER(?) AND finished = TRUE ORDER BY bestTime ASC LIMIT 1";
                ...
                try (
                    ...
                    var10000.logDatabaseOperation("Erro ao buscar tempo de " + var10001 + " na pista " + trackName + ": " + e.getMessage());
                }
            }
