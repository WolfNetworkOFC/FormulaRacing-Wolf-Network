...
public class EventCommand extends BaseCommand {
    ...
    public void onDefault(Player player) {
        Optional<Events> eventOpt = this.database.getPlayerSelectedEvent(player.getUniqueId());
        if (eventOpt.isPresent()) {
            this.onInfo(player, (Events)eventOpt.get());
        } else {
            this.plugin.sendMessage(player, "event_none_selected_hint", new String[0]);
        }
    ...
    public void onList(CommandSender sender) {
        List<Events> activeEvents = this.eventManager.getAllEvents().stream().filter(Events::isActive).sorted(Comparator.comparingLong(Events::getDate)).toList();
        sender.sendMessage("");
        this.plugin.sendMessage(sender instanceof Player ? (Player)sender : null, "event_list_header", new String[0]);
        sender.sendMessage("");
        if (activeEvents.isEmpty()) {
            this.plugin.sendMessage(sender instanceof Player ? (Player)sender : null, "event_list_empty", new String[0]);
        }
        ...
        for(Events event : activeEvents) {
            ...
            nameBtn.setHoverEvent(new HoverEvent(net.md_5.bungee.api.chat.HoverEvent.Action.SHOW_TEXT, new Content[]{new Text("§aClique para gerenciar")}));
            message.addExtra(nameBtn);
            String var10003 = String.valueOf(ChatColor.GRAY);
            message.addExtra(new TextComponent(var10003 + " (" + event.getState().name() + ")"));
            message.addExtra(new TextComponent(String.valueOf(ChatColor.DARK_GRAY) + " - "));
    ...
    public void onInfo(Player player, Events event) {
        if (event == null) {
            event = database.getPlayerSelectedEvent(player.getUniqueId()).orElse(null);
        }
    ...
    public void onCreateFull(Player player, String name, String track, @Default("15") int practiceTime, @Default("0") int qualLaps, @Default("10") int qualTime, @Default("5") int finalLaps, @Default("0") int pits) {
        if (!this.database.isCircuit(track)) {
            ...
            if (changed) {
                player.sendMessage("§e⚠️ A pista '" + track + "' não é um circuito fechado (Sprint/Parkour).");
                player.sendMessage("§e⚠️ Voltas ajustadas para 1 e Pit Stops desativados.");
            }
        }
    ...
    public void onDelete(Player player, Events event) {
        if (this.eventManager.deleteEvent(event.getId())) {
            ...
            this.plugin.sendMessage(player, "event_delete_error", new String[0]);
        }
    ...
    public void onStart(Player player, Events event) {
        ...
        if (event == null) {
            ...
            if (event.start()) {
                ...
                this.plugin.sendMessage(player, "event_start_error", new String[0]);
            }
    ...
    public void onFinish(Player player, Events event) {
        ...
        if (event == null) {
            ...
            if (event.finish()) {
                this.plugin.sendMessage(player, "event_finished", new String[0]);
            } else {
                this.plugin.sendMessage(player, "event_finish_error", new String[0]);
            }
        ...
        if (event == null) {
            ...
            if (event.isReserve(playerUUID)) {
                if (event.getState() != EventState.SETUP) {
                    this.plugin.sendMessage(player, "event_already_started_quit_error", new String[0]);
                } else {
                    if (event.removeReserve(playerUUID)) {
                        this.plugin.sendMessage(player, "event_reserve_left", new String[]{"{event}", event.getDisplayName()});
                    }

                }
            } else {
                if (event.addReserve(playerUUID)) {
                    this.plugin.sendMessage(player, "event_reserve_joined", new String[]{"{event}", event.getDisplayName()});
                    this.database.setPlayerSelectedEvent(playerUUID, event);
                } else {
                    this.plugin.sendMessage(player, "event_reserve_error", new String[0]);
                }
    ...
    public void onSigns(Player player, Events event) {
        ...
        if (event == null) {
            ...
            player.sendMessage(var10001 + String.valueOf(ChatColor.BOLD) + "  INSCRITOS: " + String.valueOf(ChatColor.WHITE) + event.getDisplayName());
            if (event.getSubscribers().isEmpty()) {
                player.sendMessage(String.valueOf(ChatColor.GRAY) + "  (Nenhum inscrito)");
            } else {
                int count = 1;

                for(Subscriber sub : event.getSubscribers().values()) {
                    String name = this.plugin.getServer().getOfflinePlayer(sub.getUuid()).getName();
                    TextComponent line = new TextComponent("  " + count + ". " + String.valueOf(ChatColor.AQUA) + name);
            ...
            if (!event.getReserves().isEmpty()) {
                ...
                player.sendMessage(var10001 + String.valueOf(ChatColor.BOLD) + "  RESERVAS:");
                int count = 1;

                for(Subscriber sub : event.getReserves().values()) {
                    String name = this.plugin.getServer().getOfflinePlayer(sub.getUuid()).getName();
                    TextComponent line = new TextComponent("  " + count + ". " + String.valueOf(ChatColor.YELLOW) + name);
                    player.spigot().sendMessage(line);
                    ++count;
                }
            }
    ...
    public void onBroadcast(Player player, Events event) {
        ...
        if (event == null) {
            ...
            player.sendMessage(String.valueOf(ChatColor.RED) + "✗ Este evento já foi finalizado!");
        } else {
            int sentCount = 0;
            ...
            this.plugin.sendMessage(player, "event_broadcast_sent", new String[]{"{count}", String.valueOf(sentCount)});
        }
    }

    private void broadcastToAdmins(String message) {
        this.plugin.getServer().getOnlinePlayers().stream().filter((p) -> p.hasPermission("formularacing.event.admin")).forEach((p) -> p.sendMessage(message));
    }
    ...
    public void onQuit(Player player) {
        ...
        if (eventOpt.isPresent()) {
            ...
            this.plugin.sendMessage(player, "event_left", new String[]{"{event}", ((Events)eventOpt.get()).getDisplayName()});
        } else {
            this.plugin.sendMessage(player, "event_not_in_event", new String[0]);
        }
    ...
    public void onSetSigns(Player player, String state) {
        ...
        if (eventOpt.isEmpty()) {
            ...
            if (open) {
                this.plugin.sendMessage(player, "event_signs_open", new String[]{"{event}", event.getDisplayName()});
            } else {
                this.plugin.sendMessage(player, "event_signs_closed", new String[]{"{event}", event.getDisplayName()});
            }
    ...
    private void showEventInfo(Player player, Events event) {
        ...
        header.addExtra(ClickableMessageUtil.getRefreshButton("/event info " + event.getDisplayName(), "Atualizar"));
        header.addExtra(new TextComponent(" "));
        TextComponent title = new TextComponent(event.getDisplayName().toUpperCase());
        title.setColor(ChatColor.GOLD);
        title.setBold(true);
        header.addExtra(title);
        String var10003 = String.valueOf(ChatColor.GRAY);
        header.addExtra(new TextComponent(var10003 + " (" + event.getState().name() + ")"));
        var10003 = String.valueOf(ChatColor.DARK_GRAY);
        header.addExtra(new TextComponent(var10003 + " (#" + event.getId() + ")"));
        ...
        if (trackName != null) {
            trackRow.addExtra(ClickableMessageUtil.getButton(trackName, ChatColor.WHITE, "/track info " + trackName, "Ver Pista", Action.RUN_COMMAND));
        } else {
        ...
        if (isAdmin) {
            ...
            trackRow.addExtra(ClickableMessageUtil.getEditButton("[Alterar]", "/event settrack " + event.getDisplayName() + " ", "Definir Pista"));
        }

        player.spigot().sendMessage(trackRow);
        TextComponent signsRow = new TextComponent(String.valueOf(ChatColor.YELLOW) + "  Inscrições: ");
        if (isAdmin) {
            if (event.isOpenSign()) {
                signsRow.addExtra(ClickableMessageUtil.getToggleButton("ABERTO", ChatColor.GREEN, "/event set signs closed", "Clique para FECHAR"));
            } else {
                signsRow.addExtra(ClickableMessageUtil.getToggleButton("FECHADO", ChatColor.RED, "/event set signs open", "Clique para ABRIR"));
            }
        } else {
            signsRow.addExtra(new TextComponent(event.isOpenSign() ? String.valueOf(ChatColor.GREEN) + "ABERTO" : String.valueOf(ChatColor.RED) + "FECHADO"));
        }

        player.spigot().sendMessage(signsRow);
        TextComponent driversRow = new TextComponent(String.valueOf(ChatColor.YELLOW) + "  Pilotos: ");
        var10003 = String.valueOf(ChatColor.WHITE);
        driversRow.addExtra(new TextComponent(var10003 + event.getSubscriberCount() + " inscritos "));
        if (event.getReserveCount() > 0) {
            var10003 = String.valueOf(ChatColor.GRAY);
            driversRow.addExtra(new TextComponent(var10003 + "(+" + event.getReserveCount() + " reservas) "));
        }

        driversRow.addExtra(ClickableMessageUtil.getButton("Ver Lista", ChatColor.AQUA, "/event signs " + event.getDisplayName(), "Ver lista completa", Action.RUN_COMMAND));
        player.spigot().sendMessage(driversRow);
        player.sendMessage("");
        String var10002 = String.valueOf(ChatColor.GOLD);
        TextComponent roundsHeader = new TextComponent(var10002 + String.valueOf(ChatColor.BOLD) + "  ROUNDS:");
        if (isAdmin) {
            roundsHeader.addExtra(new TextComponent("   "));
            roundsHeader.addExtra(ClickableMessageUtil.getButton("+ Novo Round", ChatColor.GREEN, "/round create ", "Criar novo round", Action.SUGGEST_COMMAND));
        }

        player.spigot().sendMessage(roundsHeader);
        List<Rounds> rounds = event.getSchedule().getRoundsOrdered();
        if (rounds.isEmpty()) {
            player.sendMessage(String.valueOf(ChatColor.GRAY) + "   (Nenhum round configurado)");
        } else {
            for(Rounds round : rounds) {
                String roundName = "R" + round.getRoundNumber();
                boolean isCurrent = (Boolean)event.getSchedule().getCurrentRound().map((r) -> r.getId() == round.getId()).orElse(false);
                TextComponent line = new TextComponent(isCurrent ? String.valueOf(ChatColor.YELLOW) + "  ➤ " : "    ");
                TextComponent nameComp = new TextComponent(round.getDisplayName());
                nameComp.setColor(isCurrent ? ChatColor.AQUA : ChatColor.GRAY);
                line.addExtra(nameComp);
                line.addExtra(new TextComponent(String.valueOf(ChatColor.DARK_GRAY) + " - "));
                var10003 = String.valueOf(ChatColor.WHITE);
                line.addExtra(new TextComponent(var10003 + round.getState().name()));
                line.addExtra(new TextComponent("   "));
                line.addExtra(ClickableMessageUtil.getButton("Info", ChatColor.BLUE, "/round info " + round.getId(), "Ver detalhes", Action.RUN_COMMAND));
                if (isAdmin) {
                    for(Heats heat : round.getHeats().values()) {
                        var10002 = String.valueOf(ChatColor.GRAY);
                        TextComponent heatLine = new TextComponent(var10002 + "    ↳ " + heat.getName());
                        int var10000 = round.getRoundIndex();
                        String heatRef = "R" + var10000 + (round.getType() == RoundType.QUALIFICATION ? "Q" : "F") + heat.getHeatNumber();
                        heatLine.addExtra(new TextComponent("  "));
                        TextComponent loadBtn = new TextComponent("[L]");
                        loadBtn.setColor(ChatColor.YELLOW);
                        loadBtn.setHoverEvent(new HoverEvent(net.md_5.bungee.api.chat.HoverEvent.Action.SHOW_TEXT, new Content[]{new Text("§eCarregar Heat (Grid)")}));
                        loadBtn.setClickEvent(new ClickEvent(Action.RUN_COMMAND, "/heat load " + heatRef));
                        heatLine.addExtra(loadBtn);
                        heatLine.addExtra(new TextComponent(" "));
                        TextComponent startHeatBtn = new TextComponent("[▶]");
                        startHeatBtn.setColor(ChatColor.GREEN);
                        startHeatBtn.setHoverEvent(new HoverEvent(net.md_5.bungee.api.chat.HoverEvent.Action.SHOW_TEXT, new Content[]{new Text("§aIniciar Corrida")}));
                        startHeatBtn.setClickEvent(new ClickEvent(Action.RUN_COMMAND, "/heat start " + heatRef));
                        heatLine.addExtra(startHeatBtn);
                        heatLine.addExtra(new TextComponent(" "));
                        TextComponent infoHeatBtn = new TextComponent("[>>]");
                        infoHeatBtn.setColor(ChatColor.GOLD);
                        infoHeatBtn.setHoverEvent(new HoverEvent(net.md_5.bungee.api.chat.HoverEvent.Action.SHOW_TEXT, new Content[]{new Text("§6Ver Resultados/Análise")}));
                        infoHeatBtn.setClickEvent(new ClickEvent(Action.RUN_COMMAND, "/heat info " + heatRef));
                        heatLine.addExtra(infoHeatBtn);
                        heatLine.addExtra(new TextComponent(" "));
                        TextComponent removeHeatBtn = new TextComponent("[X]");
                        removeHeatBtn.setColor(ChatColor.DARK_RED);
                        removeHeatBtn.setHoverEvent(new HoverEvent(net.md_5.bungee.api.chat.HoverEvent.Action.SHOW_TEXT, new Content[]{new Text("§4Remover Heat")}));
                        removeHeatBtn.setClickEvent(new ClickEvent(Action.RUN_COMMAND, "/heat remove " + heatRef));
        ...
        if (isAdmin) {
            ...
            addRoundBtn.setHoverEvent(new HoverEvent(net.md_5.bungee.api.chat.HoverEvent.Action.SHOW_TEXT, new Content[]{new Text("§aAdicionar novo round")}));
            addRoundBtn.setClickEvent(new ClickEvent(Action.SUGGEST_COMMAND, "/round create "));
            ...
            broadcastBtn.setHoverEvent(new HoverEvent(net.md_5.bungee.api.chat.HoverEvent.Action.SHOW_TEXT, new Content[]{new Text("§eAnunciar inscrições")}));
            broadcastBtn.setClickEvent(new ClickEvent(Action.RUN_COMMAND, "/event broadcast clicktosign " + event.getDisplayName()));
            actions.addExtra(broadcastBtn);
            actions.addExtra(new TextComponent("  "));
            TextComponent deleteBtn = new TextComponent("[Excluir Evento]");
            deleteBtn.setColor(ChatColor.RED);
            deleteBtn.setHoverEvent(new HoverEvent(net.md_5.bungee.api.chat.HoverEvent.Action.SHOW_TEXT, new Content[]{new Text("§cRemover este evento permanentemente")}));
            deleteBtn.setClickEvent(new ClickEvent(Action.RUN_COMMAND, "/event delete " + event.getDisplayName()));
