...
import dev.EfraGroup.formulaRacing.Heat.Logic.TotalChaosSession;
import dev.EfraGroup.formulaRacing.Heat.Logic.SessionLogic;

public class TotalChaosRound extends Rounds {

    public TotalChaosRound(FormulaRacing plugin, int id, Events event, int roundIndex, RoundType roundType) {
    ...
    public void broadcastResults() {
        this.plugin.getDebugManager().logRaceSystem("Anunciando resultados do Caos Total...");
    }

    @Override
    public SessionLogic getSessionLogic() {
        return new TotalChaosSession();
    }
}
