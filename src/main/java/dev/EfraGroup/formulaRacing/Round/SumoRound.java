...
import dev.EfraGroup.formulaRacing.Heat.Logic.SumoSession;
import dev.EfraGroup.formulaRacing.Heat.Logic.SessionLogic;

public class SumoRound extends Rounds {

    public SumoRound(FormulaRacing plugin, int id, Events event, int roundIndex, RoundType roundType) {
    ...
    public void broadcastResults() {
        this.plugin.getDebugManager().logRaceSystem("Anunciando resultados do Sumo...");
    }

    @Override
    public SessionLogic getSessionLogic() {
        return new SumoSession();
    }
}
